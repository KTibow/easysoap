(() => {
  ...
  function getTagAttributes(attributes = {}) {
    ...
    return " " + attributeKeys.map((key) => `${key}="${attributes[key]}"`).join(" ");
  }
  ...
  function getMethodParamRequestString(requestParams, paramKey, callParams) {
    ...
    for (const requestParamsAttributes of requestParams) {
      if (requestParamsAttributes.params) {
        methodRequestParams = requestParamsAttributes.params.find(
          (requestParamItem) =>
            requestParamItem.name === paramKey || requestParamItem.element === paramKey
        );
      }
    }
    ...
    return getParamAsString(paramKey, paramValue, methodRequestParams, mergedAttributes);
  }
  ...
  async function getRequestParamsAsString(callParams = {}, baseParams = {}, opts = {}) {
    ...
    for (const paramKey of Object.keys(callParams.params)) {
      responseArray.push(getMethodParamRequestString(requestParams, paramKey, callParams));
    }
  ...
  function getRequestHeadParams(params = {}) {
    ...
    if (!Array.isArray(params.headers) && params.headers === Object(params.headers)) {
    ...
    return params.headers.map(
      (item, index) => `<cns${index}:${item.name}>${item.value}</cns${index}:${item.name}>`
    );
  }
